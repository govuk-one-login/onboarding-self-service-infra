...
function get-source-config {
  ...
  previous_account=$(../scripts/aws.sh get-previous-account-name "$ACCOUNT")
...
INITIAL_ACCOUNT=$(../scripts/aws.sh get-initial-account "$ACCOUNT")
STACK_NAME=secure-pipelines

[[ $ACCOUNT == "$INITIAL_ACCOUNT" ]] || get-source-config
[[ $ACCOUNT == development ]] && ENV=dev || NEXT_ACCOUNT=$(../scripts/aws.sh get-next-account "$ACCOUNT")
: ${ENV:=$ACCOUNT}
